...
def run_shine_mapping_incremental():
    ...
    sigma_sigmoid = config.logistic_gaussian_ratio * \
        config.sigma_sigmoid_m*config.scale
    ...
    for frame_id in tqdm(range(dataset.total_pc_count)):
        ...
        dataset.process_frame(
            frame_id, incremental_on=config.continual_learning_reg)

        octree_feat = list(octree.parameters())
        opt = setup_optimizer(config, octree_feat,
                              geo_mlp_param, None, sigma_size)
        octree.print_detail()
        ...
        for iter in tqdm(range(config.iters)):
            ...
            sdf_loss = sdf_bce_loss(sdf_pred, sdf_label, sigma_sigmoid,
                                    weight, config.loss_weight_on, config.loss_reduction)
            cur_loss += sdf_loss
            ...
            if config.ekional_loss_on:
                ...
                eikonal_loss = (
                    (g[surface_mask].norm(2, dim=-1) - 1.0) ** 2).mean()
        ...
        if processed_frame == 0 or (processed_frame+1) % config.mesh_freq_frame == 0:
            ...
            mesh_path = run_path + '/mesh/mesh_frame_' + \
                str(frame_id+1) + ".ply"
            map_path = run_path + '/map/sdf_map_frame_' + \
                str(frame_id+1) + ".ply"
            if config.mc_with_octree:  # default
                cur_mesh = mesher.recon_octree_mesh(
                    config.mc_query_level, config.mc_res_m, mesh_path, map_path, config.save_map, config.semantic_on)
            else:
                cur_mesh = mesher.recon_bbx_mesh(
                    dataset.map_bbx, config.mc_res_m, mesh_path, map_path, config.save_map, config.semantic_on)

        T3 = get_time()

        if config.o3d_vis_on:
            if vis_mesh:
                # back to the globally shifted frame for vis
                cur_mesh.transform(dataset.begin_pose_inv)
                vis.update(dataset.cur_frame_pc,
                           dataset.cur_pose_ref, cur_mesh)
        ...
        if config.wandb_vis_on:
            wandb_log_content = {'frame': processed_frame,
                                 'timing(s)/preprocess': T1-T0, 'timing(s)/mapping': T2-T1, 'timing(s)/reconstruct': T3-T2}
            wandb.log(wandb_log_content)

        processed_frame += 1

        total_time += T2-T0
        print('avg timing(s)/(preprocess + mapping)', total_time/processed_frame)
